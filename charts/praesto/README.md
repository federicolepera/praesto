...
Pin release images explicitly for v0.3.1:
...
helm install praesto ./charts/praesto \
  ...
  --create-namespace \
  --set image.tag=0.3.1 \
  --set downloader.image.tag=0.3.1 \
  --set csi.image.tag=0.3.1 \
  --set nodeAgent.image.tag=0.3.1
```
...
| `csi.image.tag` | `0.3.1` | CSI node driver image tag |
...
| `nodeAgent.image.tag` | `0.3.1` | Node-agent image tag |
